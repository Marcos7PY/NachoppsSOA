...
export function DeliveryScreen() {
  ...
  const paramMesaNumero = searchParams.get('mesaNumero');
  const paramCanal = searchParams.get('canal');

  // Stores
  const { pedidos, loading: loadingPedidos, fetch: fetchPedidos, avanzarEstado, crear: crearPedido } = usePedidosStore();
  ...
  const mesasFisicas = useMemo(() => {
    return [...mesas.filter((m) => m.numero < 90)].sort((a, b) => a.numero - b.numero);
  }, [mesas]);
  ...
  const mesaDelivery = mesas.find((m) => m.numero === 99);
  const mesaLlevar = mesas.find((m) => m.numero === 98);
  ...
  const handleCrearPedido = async (event: React.FormEvent) => {
    ...
    if (tipo !== 'SALON' && (!clienteNombre.trim() || !clienteTelefono.trim())) {
    ...
    try {
      ...
      setErrorLocal(err instanceof Error ? err.message : 'Error al crear el pedido.');
    } finally {
      setSaving(false);
    }
  };

  const getSiguienteEstado = (estado: string): { key: string; label: string } | null => {
    if (estado === 'PENDIENTE') return { key: 'EN_PREPARACION', label: 'Preparar' };
    if (estado === 'EN_PREPARACION') return { key: 'LISTO', label: 'Marcar Listo' };
    ...
    return (
      <div>
        <div className="page-h">
          <div><h1>Pedidos</h1><div className="sub">Cargando catálogo y pedidos…</div></div>
        </div>
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div className="page-h">
        <div>
          ...
          <div className="sub">{pedidosDeliveryLlevar.length} despachos activos en panel</div>
        </div>
        <span className="spacer" />
        <button className="btn btn-ghost btn-sm" onClick={() => fetchPedidos()} title="Refrescar">
          Refrescar
        </button>
      </div>
      ...
      <div className="module-grid" style={{ flex: 1, display: 'flex', gap: 18, alignItems: 'stretch' }}>
        ...
        <section className="panel" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div className="panel-h">
            ...
            <span className="badge badge-accent">{pedidosDeliveryLlevar.length} pedidos</span>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
            {pedidosDeliveryLlevar.length === 0 ? (
              <div className="empty">
                ...
                <p>Usa el formulario de la derecha para registrar un pedido rápido de salón, delivery o para llevar.</p>
              </div>
            ) : (
                          ...
                          <td style={{ maxWidth: 300 }}>
                            <strong style={{ fontSize: 13.5 }}>
                              {primerItemNotas.includes('Cliente:') 
                                ? primerItemNotas.split('|')[0].replace(/\[DELIVERY\]|\[LLEVAR\]|Cliente:/g, '').trim()
                                : `Cliente Mesa ${p.mesaNumero}`}
                            </strong>
                            <div className="note" style={{ whiteSpace: 'normal', marginTop: 4 }}>
                              {primerItemNotas.includes('Dir:') 
                                ? primerItemNotas.split('|').find(s => s.includes('Dir:'))?.replace('Dir:', '').trim()
                                : esDelivery ? 'Dirección no especificada' : 'Retiro en local'}
                            </div>
                            {p.items[0]?.notas && !p.items[0].notas.includes('Cliente:') && (
                              <div className="note">
                                <svg className="ic" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4M12 8h.01" /></svg>
                                {' '}{p.items[0].notas}
                              </div>
                            )}
                          </td>
                          <td>
                            <span className={`badge ${esDelivery ? 'badge-info' : 'badge-warn'}`}>
                              {esDelivery ? 'DELIVERY' : 'LLEVAR'}
                            </span>
                          </td>
                          <td>
                            <strong className="mono">S/ {Number(p.total).toFixed(2)}</strong>
                          </td>
                          <td>
                          ...
                          <td style={{ textAlign: 'right' }}>
                            {next ? (
                              <button
                                ...
                                onClick={() => handleAvanzarEstado(p.id, next.key)}
                              >
                                {actionLoading === p.id ? 'Cargando…' : next.label}
                              </button>
                            ) : (
                              <span className="muted" style={{ fontSize: 12 }}>Listo para cerrar</span>
                            )}
                          </td>
                        </tr>
                      );
        ...
        <aside className="module-side" style={{ width: 440, display: 'flex', flexDirection: 'column' }}>
          <section className="panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div className="panel-h">
              <h3>Registrar Pedido</h3>
            </div>
            
            <form onSubmit={handleCrearPedido} style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: 16 }}>
              {/* Selector Tipo */}
              <div className="seg" style={{ marginBottom: 12, alignSelf: 'stretch' }}>
                <button type="button" className={tipo === 'SALON' ? 'on' : ''} onClick={() => setTipo('SALON')} style={{ flex: 1 }}>Salón</button>
                <button type="button" className={tipo === 'DELIVERY' ? 'on' : ''} onClick={() => setTipo('DELIVERY')} style={{ flex: 1 }}>Delivery</button>
                <button type="button" className={tipo === 'LLEVAR' ? 'on' : ''} onClick={() => setTipo('LLEVAR')} style={{ flex: 1 }}>Para Llevar</button>
              </div>
                ...
                {tipo === 'SALON' ? (
                  <div className="field">
                    ...
                    <div className="input">
                      <select 
                        ...
                        style={{ width: '100%', height: 38, padding: '0 10px', fontSize: 13.5, background: 'var(--surface-3)', border: '1px solid var(--border)', borderRadius: 'var(--r)' }}
                      >
                  ...
                  <>
                    <div className="field">
                      ...
                      <div className="input">
                        <input required value={clienteNombre} onChange={(e) => setClienteNombre(e.target.value)} placeholder="Ej. Juan Pérez" />
                      </div>
                    </div>
                    <div className="field">
                      <label>Teléfono</label>
                      <div className="input">
                        <input required type="tel" value={clienteTelefono} onChange={(e) => setClienteTelefono(e.target.value)} placeholder="Ej. 987654321" />
                      </div>
                    </div>
                    {tipo === 'DELIVERY' && (
                      <div className="field">
                        ...
                        <div className="input">
                          <input required value={clienteDireccion} onChange={(e) => setClienteDireccion(e.target.value)} placeholder="Ej. Av. Larco 123, Dpto 402" />
                        </div>
                      </div>
                    )}
                  </>
              ...
              <div className="field" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <label>Seleccionar Ítems</label>
                
                {/* Filtro y Búsqueda */}
                <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                  <div className="input" style={{ flex: 1 }}>
                    <input value={busquedaProducto} onChange={(e) => setBusquedaProducto(e.target.value)} placeholder="Buscar producto…" style={{ height: 32 }} />
                  </div>
                  <div className="input" style={{ width: 120 }}>
                    <select value={categoriaSeleccionada} onChange={(e) => setCategoriaSeleccionada(e.target.value)} style={{ height: 32, padding: '0 8px' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 8, display: 'grid', gap: 6, maxHeight: 150 }}>
                  {productosFiltrados.length === 0 ? (
                    <div className="muted" style={{ textAlign: 'center', padding: 8 }}>No hay productos</div>
                  ) : (
                    productosFiltrados.map((prod) => {
                      const cant = carrito[prod.id]?.cantidad || 0;
                      return (
                        <div key={prod.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 8px', background: 'var(--surface-2)', borderRadius: 'var(--r-sm)' }}>
                          <div>
                            <strong>{prod.nombre}</strong>
                            <div className="muted" style={{ fontSize: 11 }}>S/ {Number(prod.precio).toFixed(2)}</div>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            {cant > 0 && (
                              <>
                                <button type="button" className="btn btn-sm btn-ghost" onClick={() => quitarDelCarrito(prod.id)} style={{ width: 24, height: 24, padding: 0 }}>-</button>
                                <span className="mono" style={{ fontWeight: 'bold' }}>{cant}</span>
                              </>
                            )}
                            <button type="button" className="btn btn-sm btn-soft" onClick={() => agregarAlCarrito(prod)} style={{ height: 24, padding: '0 8px', fontSize: 12 }}>+ Agregar</button>
                          </div>
                        </div>
                      );
                    })
              ...
              <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                ...
                <div style={{ display: 'grid', gap: 6, maxHeight: 120, overflowY: 'auto', marginBottom: 10 }}>
                  {Object.values(carrito).length === 0 ? (
                    <div className="muted" style={{ fontSize: 12 }}>El carrito está vacío.</div>
                  ) : (
                    Object.values(carrito).map((item) => (
                      <div key={item.producto.id} style={{ fontSize: 12.5, display: 'grid', gap: 4, background: 'var(--surface-3)', padding: 8, borderRadius: 'var(--r-sm)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span><strong>{item.cantidad}x</strong> {item.producto.nombre}</span>
                          <strong className="mono">S/ {(Number(item.producto.precio) * item.cantidad).toFixed(2)}</strong>
                        </div>
                        <input
                          placeholder="Notas (ej. sin salsas, término medio)"
                          value={item.notas}
                          onChange={(e) => actualizarNotasItem(item.producto.id, e.target.value)}
                          style={{ height: 24, fontSize: 11, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 4, padding: '0 6px' }}
                        />
                      </div>
                    ))
                  )}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <strong>Total:</strong>
                  <strong className="mono" style={{ fontSize: 18, color: 'var(--accent)' }}>S/ {Number(totalCarrito).toFixed(2)}</strong>
                </div>

                <button type="submit" className="btn btn-primary btn-block" disabled={saving || !online || Object.values(carrito).length === 0} style={{ height: 38 }}>
                  {saving ? 'Procesando…' : `Crear Pedido (${tipo === 'SALON' ? 'Salón' : tipo})`}
                </button>
              </div>
            </form>
          </section>

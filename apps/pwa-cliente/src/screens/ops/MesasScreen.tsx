...
export function MesasScreen() {
  ...
  const { mesas, loading, error, fetch, optimisticCambiarEstado } = useMesasStore();
  ...
  const mesasSalon = mesas.filter((m) => m.numero < 90);
  const pedidosActivosPorMesa = pedidos.reduce<Record<string, PedidoVM[]>>((acc, pedido) => {
  ...
  const mesaTieneConsumo = (mesa: MesaVM) => Boolean(mesa.cuentaAsociada || pedidosActivosPorMesa[mesa.id]?.length);
  ...
  const itemsCuentaActiva = cuentaActiva && modalMesa && cuentaActiva.mesaId === modalMesa.id
    ...
    return (
      <div>
        <div className="page-h">
          <div><h1>Mesas</h1><div className="sub">Cargando…</div></div>
        </div>
        <div className="mesa-grid">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="mesa-card" style={{ minHeight: 130 }}>
              <div className="skel" style={{ width: 60, height: 32, marginBottom: 12 }} />
  ...
  if (mesas.length === 0) {
    return (
      <div>
        ...
        <div className="empty">
          <div className="e-ic"><GridIcon /></div>
  ...
  return (
    <div>
      <div className="page-h">
        <div>
          <h1>Mesas</h1>
          <div className="sub">{mesasSalon.length} mesas · {conteos.LIBRE} libres</div>
        </div>
        <span className="spacer" />
      ...
      <div className="mesa-grid">
        {mesasFiltradas.map((mesa) => {
          ...
          const totalMesa = pedidosMesa.reduce((sum, pedido) => sum + Number(pedido.total), 0);
          const totalPendientes = pedidosMesa.reduce(
            (sum, pedido) => sum + pedido.items.filter((item) => item.estado !== 'LISTO' && item.estado !== 'ENTREGADO' && item.estado !== 'PAGADO').length,
            0,
          );
          const primerPedido = [...pedidosMesa].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())[0];
          const tieneConsumo = mesaTieneConsumo(mesa);
          const badgeKind =
            mesa.estado === 'LIBRE' ? 'badge-ok' :
            mesa.estado === 'OCUPADA' ? 'badge-accent' :
            'badge-info';
          return (
          <div
            key={mesa.id}
            ...
            onClick={() => {
              navigate(`/app/crear-pedido?mesaId=${mesa.id}&mesaNumero=${mesa.numero}&canal=SALON`);
            }}
          >
            ...
            {mesa.estado === 'OCUPADA' ? (
              tieneConsumo ? (
                <div style={{ display: 'grid', gap: 7, marginTop: 12, borderTop: '1px solid var(--border-soft)', paddingTop: 10 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                      <div className="muted" style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase' }}>Cuenta</div>
                      <div className="mono" style={{ fontWeight: 900, fontSize: 15 }}>S/ {totalMesa.toFixed(2)}</div>
                    </div>
                    <div>
                      <div className="muted" style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase' }}>Tiempo</div>
                      <div className="mono" style={{ fontWeight: 900, fontSize: 15 }}>{primerPedido?.tiempoTranscurrido ?? 'Ahora'}</div>
                    </div>
                  </div>
                  <div className="mesa-line" style={{ color: totalPendientes > 0 ? 'var(--warn-text)' : 'var(--muted)' }}>
                    {totalPendientes > 0 ? <ClockIcon /> : <CheckIcon />}
                    {totalPendientes > 0 ? `${totalPendientes} en cocina` : 'Sin pendientes'}
                  </div>
                </div>
              ) : (
              ...
              <div className="mesa-line" style={{ color: 'var(--info-text)' }}>
                <ClockIcon /> Reservada
              </div>
            ) : (
          ...
          <aside className="drawer" role="dialog">
            <div className="drawer-h">
              ...
              <span className={`badge dot ${
                modalMesa.estado === 'LIBRE' ? 'badge-ok' :
                modalMesa.estado === 'OCUPADA' ? 'badge-accent' : 'badge-info'
              }`}>{modalMesa.estadoLabel}</span>
              <button className="icon-btn" onClick={() => setModalMesa(null)} aria-label="Cerrar"><CloseIcon /></button>
            </div>
            ...
            <div className="drawer-body">
              ...
              {modalMesa.estado === 'OCUPADA' && (
                <div className="panel" style={{ marginBottom: 16 }}>
                  <div className="panel-h">
                    <h3>Cuenta Activa</h3>
                    {cuentaActiva && <span className="badge badge-info dot">{cuentaActiva.id}</span>}
                  </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ display: 'grid', gap: 6, maxHeight: 180, overflowY: 'auto', paddingRight: 4 }}>
                          {itemsCuentaActiva.map((it, idx) => (
                            <div key={it.id || idx} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, borderBottom: '1px solid var(--border-soft)', paddingBottom: 6 }}>
                              <span>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid var(--border)', paddingTop: 10, fontWeight: 'bold', fontSize: 14, marginTop: 4 }}>
                          <span>Total Consumo:</span>
                          <span className="mono" style={{ color: 'var(--accent)', fontSize: 16 }}>
                            S/ {Number(cuentaActiva.total).toFixed(2)}
                          </span>
                        </div>
                      </div>
                    ) : (
                      <div className="muted" style={{ textAlign: 'center', fontSize: 12.5, padding: 8 }}>
                        {loading ? 'Cargando consumo…' : 'Sin consumos registrados en esta sesión.'}
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Detalle de Mesa */}
              <div className="panel" style={{ marginBottom: 16 }}>
                <div className="panel-h"><h3>Detalle de mesa</h3></div>
                <div style={{ padding: '6px 16px 12px' }}>
                  ...
                  <div className="kv" style={{ borderBottom: '1px solid var(--border)' }}>
                    <span className="k">Zona</span><span className="v">{modalMesa.zona}</span>
                  </div>
                  <div className="kv" style={{ borderBottom: '1px solid var(--border)' }}>
                    <span className="k">Capacidad</span><span className="v">{modalMesa.capacidad} personas</span>
                  </div>
                  <div className="kv">
                    <span className="k">Cuenta</span>
                    <span className="v">{modalMesa.cuentaAsociada
                      ? <span className="badge badge-info dot">Activa</span>
                      : <span className="muted">Sin cuenta abierta</span>}</span>
                  </div>
                </div>
              </div>

              <div className="panel">
                <div className="panel-h"><h3>Cambiar estado</h3></div>
                <div style={{ padding: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {modalMesa.estado !== 'LIBRE' && (
                    <button className="btn btn-sm btn-ghost" disabled={!online} onClick={() => handleCambiarEstado(modalMesa, 'LIBRE')}>Marcar libre</button>
                  )}
                  {modalMesa.estado !== 'OCUPADA' && (
                    <button className="btn btn-sm btn-ghost" disabled={!online} onClick={() => handleCambiarEstado(modalMesa, 'OCUPADA')}>Marcar ocupada</button>
                  )}
                  {modalMesa.estado !== 'RESERVADA' && (
                    <button className="btn btn-sm btn-ghost" disabled={!online} onClick={() => handleCambiarEstado(modalMesa, 'RESERVADA')}>Marcar reservada</button>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="drawer-foot">
              {modalMesa.estado === 'OCUPADA' && (
                <button className="btn btn-ghost btn-block" onClick={() => {
                  const m = modalMesa; setModalMesa(null);
                  navigate(`/app/caja?mesaId=${m.id}`);
                }}>Ver cuenta / Caja</button>
              )}
              <button
                className="btn btn-primary btn-block"
                onClick={() => {
                  const m = modalMesa; setModalMesa(null);
                  navigate(`/app/crear-pedido?mesaId=${m.id}&mesaNumero=${m.numero}&canal=SALON`);
                }}
              >
                <PlusIcon /> {modalMesa.estado === 'OCUPADA' ? 'Agregar pedido' : 'Abrir cuenta'}
              </button>
            </div>
          </aside>
        </>

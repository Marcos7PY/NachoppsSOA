...
function makeEvent(overrides: Record<string, unknown> = {}) {
  return {
    id: 'evt-1',
    routingKey: 'caja.movimiento.creado',
    payload: JSON.stringify({ movimientoId: 'mov-1' }),
    attempts: 0,
    status: 'PENDING',
...
describe('OutboxProcessor (Caja) — metadatos de cron', () => {
...
describe('OutboxProcessor (Caja) — refreshOutboxMetrics', () => {
  it('consulta pending y failed counts sin lanzar', async () => {
    const { prisma, processor } = createProcessor();
    prisma.outboxEvent.count.mockResolvedValueOnce(1).mockResolvedValueOnce(0);
...
describe('OutboxProcessor (Caja) — processOutboxEvents', () => {
  ...
  it('publica el evento y lo marca PROCESSED', async () => {
    ...
    await processor.processOutboxEvents();
    expect(rabbitmq.publish).toHaveBeenCalledWith('caja.movimiento.creado', { movimientoId: 'mov-1' }, 'servicio-caja');
  ...
  it('fallo en MAX_ATTEMPTS marca FAILED y llama notifyOutboxFailed', async () => {
    ...
    expect(notifyOutboxFailed).toHaveBeenCalledWith('servicio-caja', 'evt-1', 'caja.movimiento.creado', 5);
  });
...
describe('OutboxProcessor (Caja) — purgarOutbox', () => {
